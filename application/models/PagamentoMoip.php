...
class PagamentoMoip {
    ...
    public function generateXML(Cliente $cliente, Carrinho $carrinho, $pedido_id, $frete='Sedex') {
        ...
        $logradouro = $xml->createElement('Logradouro', $cliente->getEndereco());
        ...
        $telefonefixo = $xml->createElement('TelefoneFixo', $cliente->getTelefone());
        ...
        foreach ($allCarrinho as $p) {
            ...
            $mensagem = $xml->createElement('Mensagem', $p['qtd'] . ' - ' . $p['produto']->getNome() . ' - ' . Zend_Registry::get('currency')->toCurrency($p['produto']->getPreco()));
